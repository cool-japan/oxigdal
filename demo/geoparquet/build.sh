#!/usr/bin/env bash
#
# OxiGDAL GeoParquet Live — WASM build script.
#
# Builds crates/oxigdal-wasm-geoparquet with wasm-pack (web target) and
...
CRATE_DIR="${REPO_ROOT}/crates/oxigdal-wasm-geoparquet"
PKG_DIR="${CRATE_DIR}/pkg"
NPM_NAME="@cooljapan/oxigdal-geoparquet"
...
wasm-pack build "${CRATE_DIR}" \
    ...
    --release \
    --out-dir pkg \
    --out-name oxigdal_geoparquet

# Rewrite the package name (wasm-pack derives "oxigdal-wasm-geoparquet"
...
WASM="${PKG_DIR}/oxigdal_geoparquet_bg.wasm"
SIZE=$(wc -c < "${WASM}" | tr -d ' ')
