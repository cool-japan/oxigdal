//! Create a test COG file for Zambia Iron Belt region
//!
//! This example demonstrates creating a Cloud Optimized GeoTIFF (COG) file
//! with geographic coordinates centered on the Zambia/Congo Iron Belt mining region.
//!
//! Usage:
//!     cargo run --example create_test_cog_iron_belt
//!
//! Output:
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("Creating test COG for Zambia Iron Belt region...");

    // Zambia Iron Belt coordinates (Ndola area, near Congo border)
    // This is one of the world's largest iron mining regions
    let center_lat = -12.9638;  // South latitude
    let center_lon = 28.6362;   // East longitude
    ...
    // 30m ≈ 0.00027 degrees at latitude ~13°S
    ...
    println!("Center (at pixel 0,0): {:.6}, {:.6}", center_lat, center_lon);
    println!("Bounds: {:.6} to {:.6} (lon), {:.6} to {:.6} (lat)",
    ...
    println!("Validation: {}", if validation.is_valid { "VALID" } else { "INVALID" });
