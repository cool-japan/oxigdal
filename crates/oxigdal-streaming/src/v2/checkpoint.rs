...
impl CheckpointState {
    ...
    /// Deserialise a `CheckpointState` from bytes previously produced by [`serialize`].
    ///
...
///
/// Call [`on_event`] after processing each event.  When the cumulative sequence
...
mod tests {
    ...
    fn test_store_multiple_streams_independent() {
        ...
        assert_eq!(store.latest("stream-a").unwrap().id.sequence, 3);
        assert_eq!(store.latest("stream-b").unwrap().id.sequence, 30);
    }
