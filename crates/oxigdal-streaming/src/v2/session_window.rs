...
///
/// Call [`process`] for each incoming event (events **must** arrive in
/// non-decreasing timestamp order). Call [`flush`] at end-of-stream to close
/// any pending session. Completed windows are collected via [`drain_sessions`].
