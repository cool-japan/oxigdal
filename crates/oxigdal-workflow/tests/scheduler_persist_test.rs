...
use oxigdal_workflow::dag::WorkflowDag;
use oxigdal_workflow::scheduler::{ScheduleType, ScheduledWorkflow};
use oxigdal_workflow::{Scheduler, SchedulerConfig, WorkflowDefinition};
...
fn tmp_jsonl_path(test_name: &str) -> PathBuf {
    std::env::temp_dir().join(format!("oxigdal_scheduler_persist_{}.jsonl", test_name))
}
...
async fn test_load_state_skips_corrupt_lines() {
    ...
    let valid_wf = ScheduledWorkflow {
        ...
        max_history: 100,
        metadata: oxigdal_workflow::scheduler::ScheduleMetadata {
