...
> **Purpose:** Pure Rust MBTiles tile archive reader for OxiGDAL — SQLite-based tile pyramid support.
...
- [ ] Real SQLite-backed MBTiles file writer
  ...
  - **Prerequisites:** `oxigdal-gpkg` writer (sibling crate, sibling slice).
...
- [x] MBTiles 1.3 metadata compliance validator
  ...
  - **Design:** Mirror the `oxigdal-geojson::GeoJsonValidator` pattern: `enum ValidationIssue { MissingKey(String), InvalidBounds(String), …}` with `IssueSeverity { Error, Warning, Info }`. Validate against MBTiles 1.3 §Specification.
...
- [ ] Multi-resolution pyramid builder from a single source raster
  ...
  - **Why deferred:** Couples to `oxigdal-geotiff` for raster IO.
...
- [ ] MBTiles → PMTiles conversion via `oxigdal-pmtiles`
  - **Goal:** Cross-format migration with deterministic tile ordering.
  - **Files:** `(new) src/pmtiles_export.rs`.
  - **Why deferred:** Pairs naturally with the PMTiles ↔ MBTiles bidirectional work in `oxigdal-pmtiles`.
...
- [ ] HTTP range-request tile serving via `oxigdal-services`
...
- **Blocks:** `oxigdal-services` (tile serving), `oxigdal-pmtiles` (cross-format conversion)
- **Blocked by:** `oxigdal-gpkg` (SQLite reader/writer reuse for High Priority items 1-2)
