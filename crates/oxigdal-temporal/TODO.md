...
- [x] Implement real BFAST (Breaks For Additive Season and Trend) change detection
  ...
  - **Tests:** 10 in `crates/oxigdal-temporal/tests/bfast_test.rs` (no-break flat; no-break pure seasonal; single abrupt break; break time within one period; trend shift; short-series graceful; 5% critical value; magnitude sign matches direction; harmonic fit recovers coeffs; period/order inference).
...
- [ ] Implement zarr store export return in DataCube::to_zarr_memory
  ...
  - **Design:** Two options: (a) Use `Arc<MemoryStore>` so writer + caller share the underlying store; clone the `Arc` before passing to `ZarrV3Writer::new`. (b) Add `ZarrV3Writer::into_store()` upstream in oxigdal-zarr; consume writer post-finalize to retrieve store. Option (a) is local-only; option (b) is correct but requires oxigdal-zarr API change.
  - **Files:** `src/timeseries/datacube.rs:847-900` (rewrite the function); possibly `~/work/oxigdal/crates/oxigdal-zarr/src/v3/writer.rs` to add `into_store(self) -> Store`.
  ...
  - **Risk:** Upstream API change to oxigdal-zarr is preferred; coordinate via `IMPLEMENT POLICY`.
...
- [ ] Implement actual lazy loading for `TimeSeriesRaster` entries
  ...
  - **Design:** Define `RasterLoader` trait: `async fn load(&self, source_path: &str) -> Result<Array3<f64>>`. Register a default `FileSystemLoader` reading GeoTIFF/NetCDF via oxigdal-geotiff / oxigdal-zarr. `TimeSeriesRaster` owns `Arc<dyn RasterLoader>`. `get_data(&mut entry)` checks `entry.data` → if `None`, dispatches to loader, caches. Provide an in-memory cache eviction policy (LRU bound) to avoid OOM on long series.
...
- [ ] Cloud-backed time series (stream rasters from S3/GCS via oxigdal-rs3gw)
  - **Goal:** `RasterLoader` impl for object-store URLs; reuse `oxigdal-rs3gw` byte-range reader.
...
- [ ] STAC catalog integration for temporal queries (oxigdal-stac integration)
...
- **Blocks:** None
- **Blocked by:** oxigdal-zarr (for `to_zarr_memory` return-store fix; option (a) local workaround exists), oxigdal-geotiff (for lazy-loading default loader)
