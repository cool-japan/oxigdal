...
impl Mollweide {
    ...
    fn solve_theta(lat_rad: f64) -> Result<f64> {
        let target = std::f64::consts::PI * lat_rad.sin();
    ...
    pub fn forward(&self, lon_deg: f64, lat_deg: f64) -> Result<(f64, f64)> {
        ...
        let x = self.radius * (2.0 * sqrt2 / std::f64::consts::PI) * d_lon * theta.cos();
    ...
    pub fn inverse(&self, x: f64, y: f64) -> Result<(f64, f64)> {
        ...
        if cos_theta.abs() < TOLERANCE {
            ...
            let sin_phi = (2.0 * theta + (2.0 * theta).sin()) / std::f64::consts::PI;
        ...
        let lat = sin_phi.clamp(-1.0, 1.0).asin().to_degrees();
        let lon = self.lon_0
            + (std::f64::consts::PI * x / (2.0 * sqrt2 * self.radius * cos_theta)).to_degrees();
...
impl EckertIV {
    ...
    fn c_const() -> f64 {
        2.0 / (std::f64::consts::PI * (4.0 + std::f64::consts::PI)).sqrt()
    }

    /// Solves  θ + sin(θ)cos(θ) + 2sin(θ) = (2 + π/2)·sin(φ)  for θ.
    fn solve_theta(lat_rad: f64) -> Result<f64> {
        let rhs = (2.0 + std::f64::consts::FRAC_PI_2) * lat_rad.sin();
    ...
    pub fn forward(&self, lon_deg: f64, lat_deg: f64) -> Result<(f64, f64)> {
        ...
        let y = 2.0 * self.radius * c * std::f64::consts::PI.sqrt() * theta.sin();
    ...
    pub fn inverse(&self, x: f64, y: f64) -> Result<(f64, f64)> {
        let c = Self::c_const();
        let sin_theta = y / (2.0 * self.radius * c * std::f64::consts::PI.sqrt());
        if sin_theta.abs() > 1.0 + TOLERANCE {
            return Err(Error::numerical_error(
...
impl EckertVI {
    ...
    /// C = sqrt(2 / (π(π+4)))
    fn c_const() -> f64 {
        (2.0 / (std::f64::consts::PI * (std::f64::consts::PI + 4.0))).sqrt()
    }

    /// Solves θ + sin(θ) = (1 + π/2)·sin(φ) for θ via Newton–Raphson.
    fn solve_theta(lat_rad: f64) -> Result<f64> {
        let rhs = (1.0 + std::f64::consts::FRAC_PI_2) * lat_rad.sin();
    ...
    pub fn inverse(&self, x: f64, y: f64) -> Result<(f64, f64)> {
        ...
        let sin_phi = (theta + theta.sin()) / (1.0 + std::f64::consts::FRAC_PI_2);
        let lat = sin_phi.clamp(-1.0, 1.0).asin().to_degrees();
