//! Integration tests for [`oxigdal_index::StreamingRTree`].
//!
//! All tests use `(Bbox2D, u32)` entries — identical to the style used in
//! `rtree_test.rs` — so no external dependencies beyond `oxigdal_index` are
//! required.

use oxigdal_index::{Bbox2D, StreamingInsertConfig, StreamingRTree};
