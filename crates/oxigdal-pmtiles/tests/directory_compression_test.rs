...
use oxigdal_pmtiles::{Compression, PmTilesBuilder, PmTilesHeader, PmTilesReader, TileType};
...
fn test_round_trip_archive_with_gzip_internal_compression_preserves_all_tiles() {
    ...
    for i in 0..tile_count {
        ...
        let (z, x, y) = oxigdal_pmtiles::tile_id_to_zxy(u64::from(i)).expect("tile_id_to_zxy");
        let got = reader
            .get_tile(z, x, y)
            .expect("get_tile")
            .expect("tile must exist");
