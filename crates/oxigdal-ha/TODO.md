...
- [ ] Real WAL replay for point-in-time recovery (currently sleeps 100 ms and returns fake count).
  ...
  - **Risk:** Without a real WAL writer in the rest of OxiGDAL, this is structurally a library — tests will use a fixture writer. Document this as "transport layer; consumers must call `WalWriter::append`".
  - **Prerequisites:** None.
...
- [ ] Snapshot-based backup to cloud object storage (S3/GCS/Azure Blob via `oxigdal_core::ObjectStore`).
...
- **Blocks:** `oxigdal-cluster` (consumes Raft + replication), `oxigdal-server` (consumes failover hooks).
- **Blocked by:** None.
