...
pub struct WasmCogViewer {
    ...
    mem_reader: Option<oxigeo_geotiff::CogReader<MemorySource>>,
}
...
fn to_js_error(err: &OxiGeoError) -> JsValue {
...
impl AdvancedCogViewer {
    ...
    pub async fn open(&mut self, url: &str, cache_size_mb: usize) -> Result<(), JsValue> {
        ...
        let header =
            oxigeo_geotiff::TiffHeader::parse(&header_bytes).map_err(|e| to_js_error(&e))?;

        // Parse the full file
        let tiff = oxigeo_geotiff::TiffFile::parse(&backend).map_err(|e| to_js_error(&e))?;

        // Get image info
        let info = oxigeo_geotiff::ImageInfo::from_ifd(
            tiff.primary_ifd(),
            &backend,
            header.byte_order,
            header.variant,
        ...
        // Get EPSG code
        if let Ok(Some(geo_keys)) = oxigeo_geotiff::geokeys::GeoKeyDirectory::from_ifd(
            tiff.primary_ifd(),
            &backend,
            header.byte_order,
            header.variant,
    ...
    async fn read_tile_internal(
        ...
        let reader = oxigeo_geotiff::CogReader::open(backend).map_err(|e| to_js_error(&e))?;
        reader
            .read_tile(level, tile_x, tile_y)
            .map_err(|e| to_js_error(&e))
    }
