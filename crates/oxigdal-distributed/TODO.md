...
- [ ] Spatial partitioning via R-tree for predicate pruning
  - **Goal:** Wire `oxigdal-index::STRtree` into `SpatialPartitioner` so coordinator can skip partitions disjoint from the query bbox.
...
- **Blocks:** oxigdal-cluster (uses oxigdal-distributed as compute substrate), oxigdal-services (would expose a "/jobs" endpoint backed by Coordinator).
