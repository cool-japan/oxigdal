...
- [ ] Device discovery via mDNS / DNS-SD (RFC 6762 + RFC 6763)
  ...
  - **Goal:** Zero-config LAN discovery — coordinator auto-discovers peers advertising `_oxigdal-sync._tcp.local.` service.
  - **Design:** Use `mdns-sd` crate (Pure Rust). Publish `ServiceInfo { service_type: "_oxigdal-sync._tcp.local.", hostname, port, txt: {device_id, version} }`. Subscribe to same service type; emit `DiscoveryEvent::PeerFound { device_id, addr }`.
...
- [ ] Partial-replication by spatial bbox (sync only a region of interest)
  ...
  - **Files:** `src/coordinator.rs`, integration with `oxigdal-index` STRtree.
...
- [ ] Bridge module for oxigdal-offline ↔ oxigdal-sync (offline-first → P2P sync).
- [ ] Multi-master PostGIS replication adapter using OR-Set tombstones.

## Cross-crate dependencies
- **Blocks:** oxigdal-offline (sync engine plug-in), oxigdal-edge (device-to-device fleet sync), oxigdal-mobile (mobile-app sync layer).
- **Blocked by:** None.
