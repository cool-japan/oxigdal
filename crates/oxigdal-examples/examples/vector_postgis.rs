...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let bbox = oxigdal_core::types::BoundingBox::new(-10.0, 40.0, -9.0, 41.0)?;
...
async fn run_roundtrip(config: &ConnectionConfig) -> Result<usize, Box<dyn std::error::Error>> {
    ...
    let mut writer = PostGisWriter::new(write_pool, "oxigdal_examples_pois")
        .srid(4326)
        .create_table(true);
    ...
    let mut reader = PostGisReader::new(read_pool, "oxigdal_examples_pois");
