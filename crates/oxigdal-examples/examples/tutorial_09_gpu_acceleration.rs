...
//!   (`oxigdal_gpu::cpu_fallback::cpu`) when no GPU is available
...
//! fallback kernels, which are the same code path `oxigdal-gpu` itself uses
//! when a GPU operation is unavailable or fails.

use oxigdal_core::buffer::RasterBuffer;
use oxigdal_core::types::RasterDataType;
use oxigdal_gpu::GpuContext;
use oxigdal_gpu::cpu_fallback::cpu;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match GpuContext::new().await {
        ...
        Err(e) => {
            ...
            println!("Falling back to CPU kernels (`oxigdal_gpu::cpu_fallback::cpu`)");
        }
    }
    ...
    println!("  - `oxigdal_gpu::cpu_fallback::cpu` provides the same primitive ops on CPU");
