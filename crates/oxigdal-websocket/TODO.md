...
- [ ] Wire `GeoBboxFilter::should_deliver` to actually consult message coordinates (currently accepts all)
  ...
  - **Design:** Extend `Payload` (see `src/protocol/message.rs` — `Message { id, msg_type, timestamp, payload, correlation_id }`) with an optional `coord: Option<(f64, f64)>` extractor or a typed `Payload::Feature { geometry: oxigdal_geo::Geometry, ... }` variant. `should_deliver` then runs `self.contains(coord.0, coord.1)` from the existing `GeoBboxFilter::contains(&self, x: f64, y: f64) -> bool` at `src/broadcast/filter.rs:140-142`. For non-spatial payloads, default to "deliver" (preserve current behaviour for non-feature traffic).
...
- [ ] Add per-connection authentication on the WebSocket upgrade (JWT / API-key validation)
  ...
  - **Design:** Replace `accept_async(stream)` with `tokio_tungstenite::accept_hdr_async(stream, callback)`; callback inspects request headers, validates JWT via `jsonwebtoken` (workspace) or `oxigdal-auth` if available, and returns `Err(Response::builder().status(401).body(Some("unauthorized")).build())` on failure. Add `ServerConfig::auth: AuthMode { None | Jwt(SecretKey) | ApiKey(HashSet<String>) }`.
...
- **Blocked by:** None
- **Sibling:** oxigdal-ws (axum-based; this crate is raw tokio-tungstenite — keep both, do not duplicate functionality between them)
