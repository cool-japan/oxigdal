...
use oxigdal_gpkg::{GeoPackage, multi_geom};
...
use oxigdal_zarr::{FilesystemStore, Store, StoreKey};
use serde::Serialize;
...
fn read_flatgeobuf_info(args: &InfoArgs) -> Result<VectorInfo> {
    let file = File::open(&args.input)
        .with_context(|| format!("Failed to open file: {}", args.input.display()))?;
    let buf_reader = BufReader::new(file);
    let reader = oxigdal_flatgeobuf::FlatGeobufReader::new(buf_reader)
...
fn flatgeobuf_crs_label(crs: &oxigdal_flatgeobuf::CrsInfo) -> String {
...
fn read_zarr_info(args: &InfoArgs) -> Result<ArrayInfo> {
    ...
    if store.exists(&v3_key).unwrap_or(false) {
        ...
        let meta: oxigdal_zarr::metadata::v3::ArrayMetadataV3 = serde_json::from_slice(&bytes)
    ...
    if store.exists(&v2_key).unwrap_or(false) {
        ...
        let meta: oxigdal_zarr::metadata::v2::ArrayMetadataV2 = serde_json::from_slice(&bytes)
...
fn read_gpkg_srs_map(gp: &GeoPackage) -> HashMap<i32, String> {
    ...
    for (_rowid, values) in &rows {
        ...
        let srs_id = match &values[1] {
            oxigdal_gpkg::CellValue::Integer(i) => *i as i32,
            _ => continue,
        };
        ...
        let org_code = match &values[3] {
            oxigdal_gpkg::CellValue::Integer(i) => *i,
            _ => continue,
        };
...
mod tests {
    ...
    fn test_flatgeobuf_crs_label_org_code() {
        let crs = oxigdal_flatgeobuf::CrsInfo::from_epsg(4326);
        assert_eq!(flatgeobuf_crs_label(&crs), "EPSG:4326");
    }

    #[test]
    fn test_flatgeobuf_crs_label_wkt_fallback() {
        let crs = oxigdal_flatgeobuf::CrsInfo::from_wkt("GEOGCS[\"WGS 84\"]");
