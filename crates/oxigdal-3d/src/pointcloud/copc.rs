...
/// Convert an [`oxigdal_copc::Point3D`] into the crate-native [`Point`].
///
/// COPC formats 6-10 carry RGB (7/8) and NIR (8/10); the current oxigdal-copc
/// point model surfaces RGB but not NIR, so `nir` is left `None`.
fn copc_point_to_point(p: &oxigdal_copc::Point3D) -> Point {
...
/// pure-Rust LAZ decompressor ([`oxigdal_copc::decompress_chunk`]) before the
...
fn decode_copc_chunk(
    ...
    let raw_records: std::borrow::Cow<'_, [u8]> = if decode.is_laz {
        let decompressed = oxigdal_copc::decompress_chunk(
            chunk,
            point_count,
    ...
    let points3d = oxigdal_copc::deserialize_points(
        &raw_records,
        point_count,
        decode.record_length,
        decode.format_id,
...
/// Render an [`oxigdal_copc::LasVersion`] as a `"major.minor"` string.
#[cfg(feature = "async")]
fn las_version_string(v: oxigdal_copc::LasVersion) -> String {
    use oxigdal_copc::LasVersion;
    match v {
        LasVersion::V10 => "1.0",
        LasVersion::V11 => "1.1",
        LasVersion::V12 => "1.2",
...
impl CopcReader {
    ...
    pub fn open<P: AsRef<Path>>(path: P) -> Result<Self> {
        ...
        // via oxigdal-copc so the decode parameters (format id, record length,
    ...
    fn parse_decode_params(file: &mut File) -> Result<ChunkDecodeParams> {
        ...
        let copc_header = oxigdal_copc::LasHeader::parse(&preamble)
        ...
        let is_laz = match oxigdal_copc::vlr_chain::parse_vlrs(&vlr_region, &copc_header) {
            Ok(vlrs) => oxigdal_copc::detect_laszip_vlr(&vlrs).is_some(),
...
impl CopcHttpReader {
    ...
    pub async fn open(url: impl Into<String>) -> Result<Self> {
        ...
        let copc_header = oxigdal_copc::LasHeader::parse(&head)
            .map_err(|e| Error::Copc(format!("failed to parse LAS header: {e}")))?;
        ...
        let vlrs = oxigdal_copc::vlr_chain::parse_vlrs(&vlr_region, &copc_header)
            .map_err(|e| Error::Copc(format!("failed to parse VLR chain: {e}")))?;
        let copc_info = oxigdal_copc::vlr_chain::find_copc_info(&vlrs)
            .map_err(|e| Error::Copc(format!("missing COPC info VLR: {e}")))?;
        let is_laz = oxigdal_copc::detect_laszip_vlr(&vlrs).is_some();
...
mod decode_tests {
    ...
    //!
    //! These exercise the real wiring into `oxigdal-copc`: an uncompressed raw
    //! LAS chunk and a genuinely LAZ-compressed chunk (produced by the
    //! `oxigdal-copc` encoder) are both decoded through the exact same code path
    ...
    fn decode_laz_compressed_chunk_round_trips() {
        ...
        // oxigdal-copc encoder, then decode through the production path.
        ...
        let compressed = oxigdal_copc::laz::format_v1::compress_format_0(&raw, records.len());
