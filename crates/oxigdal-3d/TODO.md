...
- [x] Real COPC reader — parse COPC VLR + hierarchy pages
  ...
  - **Files:** `crates/oxigdal-3d/src/pointcloud/copc.rs` (replace lines 286-307 and 405-410 with real impl).
  ...
  - **Tests:** 10 in `crates/oxigdal-3d/tests/copc_vlr_test.rs` (canonical 160-byte payload; wrong-length errors; LE round trip; find-VLR matching; find-VLR returns None; hierarchy single entry; hierarchy multiple entries decoded in order; voxel-key extraction; nested page walk via negative byte_size; missing-VLR typed error). Full crate suite 100/100 (90 pre-existing + 10 new).
...
- [ ] LAZ decompression in Pure Rust (replace placeholder dispatch)
  ...
  - **Files:** `crates/oxigdal-3d/src/pointcloud/copc.rs` (replace `read_voxel` decompression paths), `src/pointcloud/ept.rs` (replace the format dispatch).
...
- [ ] Delaunay triangulation for TIN generation from point clouds
  ...
  - **Files:** `crates/oxigdal-3d/src/terrain/tin.rs` (extend or new).
...
- [x] Proper planarity in ground/feature classification
  ...
  - **Files:** `crates/oxigdal-3d/src/classification.rs` (replace line 300 logic).
...
- [ ] glTF 2.0 / GLB binary export with embedded textures
  ...
  - **Files:** `crates/oxigdal-3d/src/mesh/gltf.rs` (extend existing `export_gltf`).
...
- [ ] 3D Tiles (Cesium) tileset.json with content hierarchy
  ...
  - **Files:** `crates/oxigdal-3d/src/visualization/tileset.rs` (extend or new).
...
- **Blocks:** oxigdal-copc (consumer of COPC reader), oxigdal-services (3D tile serving).
