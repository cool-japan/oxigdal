...
- [ ] Replace `(example)` placeholder responses in magic-command execution with real OxiGDAL calls
  ...
  - **Goal:** `%crs raster` returns the dataset's real CRS via `oxigdal-proj`; `%bounds raster` returns true `(minx, miny, maxx, maxy)` from the dataset's GeoTransform + dimensions; `%stats raster [band]` returns real `min/max/mean/stddev/median` computed by `oxigdal_algorithms::statistics`.
  - **Design:** Change `Value::Path(PathBuf)` (the current namespace entry from `%load_raster`) to `Value::Dataset(Arc<oxigdal_core::Dataset>)`. In `%load_raster`, open the file via `oxigdal_geotiff::GeoTiffReader::open` and store the opened dataset rather than just the path. For `%crs`: pull `Crs` from `Dataset::crs()`. For `%bounds`: compute `[geotransform.x_at(0,0), .y_at(0,h), .x_at(w,0), .y_at(0,0)]`. For `%stats`: call `oxigdal_algorithms::statistics::band_statistics(dataset, band, BandStatsOptions::default())`. Mirror the rasterio API where reasonable.
  - **Files:** `crates/oxigdal-jupyter/src/magic.rs` (replace 3 `(example)` arms); `crates/oxigdal-jupyter/src/kernel.rs` (extend `Value` enum); (new) `crates/oxigdal-jupyter/src/dataset_value.rs`.
  ...
  - **Prerequisites:** None — oxigdal-geotiff, oxigdal-proj, oxigdal-algorithms::statistics are workspace deps.

- [ ] Wire `evcxr` integration so the kernel is actually a Jupyter kernel
  - **Verified gap:** `Cargo.toml:25` — `evcxr.workspace = true`, but `rg evcxr crates/oxigdal-jupyter/src` returns zero usages. The current `OxiGdalKernel::execute` (in `src/kernel.rs:128`) parses `let name = value` lines manually and stores results in a HashMap — this is not the Jupyter messaging protocol (v5.4 ZMQ multipart) and cannot be launched as a real `jupyter console --kernel oxigdal` kernel.
  - **Goal:** Either (a) integrate `evcxr_jupyter` as the actual kernel binary so users get a real Rust Jupyter kernel with OxiGDAL preloaded, or (b) implement the Jupyter messaging protocol v5.4 directly via `zmq` for our custom kernel. Path (a) is recommended — evcxr already speaks ZMQ.
  - **Design:** Add an `oxigdal-jupyter-kernel` binary target that runs `evcxr_jupyter::run_jupyter_kernel(args)` with our preloaded extern crate list. Provide a `kernel.json` at install time pointing to this binary. Keep the existing `OxiGdalKernel` struct as a *namespace cache* for the REPL — rename to `OxiGdalContext` and downgrade from the kernel role. Implement `jupyter messaging protocol` v5.4 message types: `kernel_info_request`/`reply`, `execute_request`/`reply`, `display_data`, `comm_open`/`comm_msg`/`comm_close` for widgets.
  - **Files:** (new) `crates/oxigdal-jupyter/src/bin/oxigdal_kernel.rs`; (new) `crates/oxigdal-jupyter/kernel/kernel.json` (template); rename `crates/oxigdal-jupyter/src/kernel.rs` → keep but downgrade type.
...
- [ ] Implement Leaflet/MapLibre `MapWidget` rendering via Jupyter comm messages
  ...
  - **Design:** Use `ipywidgets`-compatible widget protocol: emit `application/vnd.jupyter.widget-state+json` payload (`widget_id`, `model_name="OxiGdalMapModel"`, initial state). Ship a small companion JS package `oxigdal-jupyter-widgets` (under `crates/oxigdal-jupyter/js/`) that registers the model + view. Backend `comm_open` creates the bidirectional channel; messages serialize `{ "method": "set_center", "lat": ..., "lng": ... }`. For static (non-Jupyter) rendering, fall back to a self-contained HTML string with embedded Leaflet from CDN.
  - **Files:** `crates/oxigdal-jupyter/src/widgets.rs` (extend `MapWidget::render` and add comm hooks); (new) `crates/oxigdal-jupyter/js/` (npm package scaffold); (new) `crates/oxigdal-jupyter/src/comm.rs` (comm-msg protocol).
...
- [ ] Rich HTML/SVG/PNG display for raster bands (RichDisplay implementation)
  - **Verified gap:** Existing TODO line — `[ ] Implement rich HTML/SVG display for raster band visualization`. `src/display.rs` (17.4K) declares `RichDisplay` and `DisplayData` types but `rg "image/png" crates/oxigdal-jupyter/src/display.rs` likely returns zero — verify the actual data emission path is wired to `plotters` (which is a workspace dep).
  - **Goal:** `dataset.display()` emits a real PNG image of the raster (using plotters' Bitmap backend or oxigdal-webp encoder) via `application/vnd.jupyter.display_data` with mimetype `image/png`. Multi-band rasters render as RGB composites; single-band rasters apply a colormap (viridis default).
  - **Design:** Implement `RichDisplay::for_raster(dataset: &Dataset, options: RasterDisplayOptions) -> DisplayData`. Internals: read the raster bands → optionally apply colormap from `oxigdal_algorithms::colormap` (existing) → use `plotters::backend::BitMapBackend` to draw → encode to PNG via `image` crate (already in deps) → base64-encode → wrap in `{ "image/png": "<base64>", "text/plain": "<W>x<H> raster" }`. Provide SVG path for vector overlays via `plotters::backend::SVGBackend`. Provide low-res "thumbnail" mode for large rasters.
  - **Files:** `crates/oxigdal-jupyter/src/display.rs` (real PNG/SVG emission); `crates/oxigdal-jupyter/src/plotting.rs` (14.0K, extend); (new) `crates/oxigdal-jupyter/src/raster_display.rs`.
...
- [ ] `%load_raster` actually opens a real Dataset (not just stores the path)
  ...
  - **Design:** Couples to Item 1 (real magic commands). Open the file via `oxigdal::open` dispatcher (multi-format) so it works for GeoTIFF/GeoParquet/GeoPackage. Time the open. Report shape + dtype. On error, return `JupyterError::Magic` with the underlying `OxiGdalError` chain.
  - **Files:** `crates/oxigdal-jupyter/src/magic.rs` (LoadRaster arm); `crates/oxigdal-jupyter/src/kernel.rs` (`Value::Dataset` variant); modify `Cargo.toml` to add `oxigdal-oxigdal` (umbrella) workspace dep for the multi-format dispatcher.
  ...
  - **Risk:** Adding the umbrella `oxigdal-oxigdal` dep creates a tighter coupling — verify no cycle.
...
- [ ] Inline histogram + statistics tile rendered alongside `display(raster)`
  ...
  - **Files:** `crates/oxigdal-jupyter/src/display.rs` (extend).
...
- [ ] `%crs_info <dataset>` extended magic with PROJ.4 / WKT2 / EPSG fields
  ...
  - **Files:** `crates/oxigdal-jupyter/src/magic.rs` (new variant).
...
- [ ] Interactive polygon-drawing widget for ROI selection
  ...
  - **Files:** `crates/oxigdal-jupyter/src/widgets.rs` (add `PolygonDrawWidget`).
...
- [ ] Tab-completion for OxiGDAL functions and dataset properties
  - **Goal:** `complete()` in `kernel.rs` extended with dataset property names (`.crs`, `.bounds`, `.bands`).
  - **Files:** `crates/oxigdal-jupyter/src/kernel.rs` (`complete` method).
...
- [ ] Kernel interrupt handling for cancelling long ops
  ...
  - **Files:** `crates/oxigdal-jupyter/src/kernel.rs` (and new bin target).
...
- [ ] GPU memory monitoring widget for oxigdal-gpu sessions.
- [ ] Automatic code generation from widget state changes (`%capture`).
- [ ] Google Colab integration (`oxigdal-colab` package).
- [ ] AWS SageMaker kernel image.

## Cross-crate dependencies
- **Blocks:** oxigdal-python (`%oxigdal_load` would delegate here).
- **Blocked by:** oxigdal-algorithms::statistics (band stats), oxigdal-proj (CRS strings), oxigdal-oxigdal umbrella (multi-format `open()`).
