...
- [ ] Implement `Expr::Function` evaluation in the filter executor (currently unreachable for any spatial WHERE clause).
  ...
  - **Risk:** WKT vs WKB literal handling — `Literal::String("POINT(1 2)")` needs parsing into geometry; reuse `wkt` crate (already in `oxigdal-db-connectors` deps; promote to workspace).
  - **Prerequisites:** None.
  ...
  - **Tests:** 20 (4 inline unit + 16 in `crates/oxigdal-query/tests/spatial_funcs_test.rs`) — covers every predicate + metric + constructor + error path (unknown name, wrong arity, type mismatch) + end-to-end executor integration.
...
- [ ] OGC CQL2 (OGC 21-065) parser alongside SQL.
  ...
  - **Files:** New `src/parser/cql2.rs`. Sibling crate `oxigdal-services` already has a CQL subset (`ogc_features/cql.rs`); consider extracting.
...
- [ ] Distributed query execution across multiple nodes (`oxigdal-cluster` integration).
...
- **Blocks:** `oxigdal-services` (when WFS GetFeature dispatches via query engine).
- **Blocked by:** `oxigdal-index` (provides rstar wrappers), `oxigdal-postgis` (provides PostGIS `DataSource`).
