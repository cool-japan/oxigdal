...
- [ ] Native complex dtype (`complex64`/`complex128`) for `RasterArray::to_numpy`
  ...
  - **Design:** PyO3 0.28 has `numpy::Complex32` / `numpy::Complex64` as of `numpy` crate v0.27 (re-exported `num_complex::Complex<f32/f64>`). Use `PyArray2::<Complex64>::from_owned_array_bound` from a `ndarray::Array2<Complex64>` built from the existing interleaved buffer. Add a sniff step that detects `Complex32` / `Complex64` in the source `RasterDataType` enum (already exists in `oxigdal_core::types::RasterDataType::CFloat32`/`CFloat64`).
  - **Files:** `crates/oxigdal-python/src/numpy.rs` (`to_numpy_complex`); `crates/oxigdal-python/src/array.rs` (dtype mapping); `crates/oxigdal-python/oxigdal.pyi` (declare `complex64`/`complex128` return types).
...
- [ ] Promote `pyo3-asyncio` integration from TODO comment to real implementation
  - **Verified gap:** `crates/oxigdal-python/Cargo.toml:40-44` — `# TODO: Add async support when pyo3-asyncio 0.23 is released` / `# pyo3-asyncio = { version = "0.23", features = ["tokio-runtime"], optional = true }` / `# tokio = { workspace = true, optional = true }` / `# futures = { workspace = true, optional = true }` / `# async-trait = { workspace = true, optional = true }`
  ...
  - **Files:** `crates/oxigdal-python/Cargo.toml` (uncomment + change crate name); (new) `crates/oxigdal-python/src/async_io.rs`; modify `crates/oxigdal-python/src/dataset.rs` to expose `async fn` mirrors.
...
- [ ] HTTP range-request COG reader exposed as `oxigdal.open(url)` for s3://, https://
  - **Verified gap:** Existing TODO line — `[ ] Add actual raster file I/O connecting to oxigdal-geotiff reader/writer` (partially done — `Dataset::open` works for local paths) and `[ ] Implement windowed reading API for processing rasters larger than memory`. No remote URL handling in `src/dataset.rs` today (verified — see `src/dataset.rs:1-80`).
  - **Goal:** `oxigdal.open("https://...")` and `oxigdal.open("s3://bucket/key")` return a Dataset backed by the same `AsyncDataSource` used by oxigdal-cloud, with windowed `ds.read_window(Window(col_off, row_off, w, h))` for >RAM rasters.
  - **Design:** Detect URL prefix in `Dataset::open` and dispatch to `oxigdal_cloud::HttpDataSource` (gated by the existing `cloud` feature in Cargo.toml). Provide a sync wrapper that runs a private `tokio::runtime::Runtime` inside the `Dataset` struct (one runtime per Python process, reused across calls). Windowed read API takes a Python `Window` namedtuple and returns a `ndarray::Array2` view.
  - **Files:** `crates/oxigdal-python/src/dataset.rs` (URL dispatch + windowed read); `crates/oxigdal-python/src/raster/operations.rs` (windowed plumbing); `crates/oxigdal-python/Cargo.toml` (enable `cloud` in default features once stable).
  ...
  - **Prerequisites:** oxigdal-cloud `HttpDataSource` (already exists per workspace).
...
- [ ] Replace the EPSG parser regex matchstring with structured parse-and-error reporting
  ...
  - **Design:** Single dispatcher `enum CrsSpec { Epsg(u32), Ogc(String), Wkt(String), Proj4(String) }`. EPSG regex: `^\s*(?i:epsg)\s*:\s*([1-9]\d{0,5})\s*$`. WKT sniffed by `^\s*(GEOG|PROJ|COMPOUND|VERT|ENGINEERING|GEODETIC)CRS\b`. PROJ.4 by `^\s*\+proj=`. Map each variant through `oxigdal_proj::Crs::from_*`. Return `PyValueError` with a Hint message naming the detected format.
  - **Files:** `crates/oxigdal-python/src/raster/core_ops.rs` (replace stringly-typed parser); `crates/oxigdal-python/src/vector/helpers.rs` (delegate to shared parser); (new) `crates/oxigdal-python/src/crs_parse.rs` (single source of truth).
...
- [ ] Auto-generate `oxigdal.pyi` type stubs from PyO3 signatures
  - **Verified gap:** Existing TODO — `[ ] Add type stub (.pyi) auto-generation for IDE completion support`. The current `oxigdal.pyi` (13.9K) is hand-maintained; drift from the actual Rust signatures is likely.
  - **Goal:** A `cargo xtask gen-pyi` step that re-emits `oxigdal.pyi` from PyO3 macros so the type stub never drifts. Run as a pre-publish gate.
  - **Design:** Use `pyo3-stub-gen 0.9` (the canonical generator; ships an attribute `#[gen_stub_pyfunction]` and a binary `pyo3-stub-gen`). Annotate every `#[pyfunction]` and `#[pyclass]`. Generated file lives at `crates/oxigdal-python/oxigdal.pyi` (overwrite). Add a CI guard that fails if `git diff` shows changes after running the generator.
  - **Files:** `crates/oxigdal-python/Cargo.toml` (add `pyo3-stub-gen` dep); annotate every `#[pyfunction]` site under `crates/oxigdal-python/src/`; (new) `crates/oxigdal-python/src/bin/gen_pyi.rs` runner.
  - **Tests:** (proposed) `test_gen_pyi_emits_oxigdal_pyi_with_open_signature`, `test_pyi_file_in_sync_with_macros` (integration), `test_open_signature_matches_rust`.
...
## Medium Priority
- [ ] rasterio-compatible facade (`rasterio.open` → `oxigdal.open`)
  - **Goal:** Drop-in replacement layer for the most-used rasterio APIs (`open`, `Window`, `read`, `write`, `transform`, `crs`).
  - **Files:** (new) `crates/oxigdal-python/python/oxigdal/_rasterio_compat.py`.
...
- [ ] geopandas `__geo_interface__` bidirectional conversion
  - **Goal:** Accept a GeoDataFrame in `oxigdal.write_geojson`; return GeoDataFrame from `read_geojson(as_gdf=True)`.
  - **Files:** `crates/oxigdal-python/src/vector/` (extend GeoJSON path).
  - **Why deferred:** Coordinated with item below (Arrow exchange).

- [ ] xarray DataArray adapter (`oxigdal.read(...).to_xarray()`)
  - **Goal:** Multi-band raster as a labeled (band, y, x) DataArray with CRS/transform on `.attrs`.
  - **Files:** (new) `crates/oxigdal-python/python/oxigdal/_xarray_adapter.py`.
  - **Why deferred:** Pure-Python wrapper; can ship without Rust changes.

- [ ] Point cloud LAS/LAZ bindings (via oxigdal-copc)
  - **Goal:** `oxigdal.open("file.las")` returns a Python PointCloud with NumPy structured array view.
  - **Files:** (new) `crates/oxigdal-python/src/pointcloud.rs`.
  - **Why deferred:** Lower demand than raster work.

- [ ] GeoPackage vector read/write bindings (via oxigdal-gpkg)
  - **Goal:** `oxigdal.read_gpkg(path, layer=...)` → Feature iterator, `write_gpkg(path, features, layer=...)`.
  - **Files:** (new) `crates/oxigdal-python/src/gpkg.rs`.
  - **Why deferred:** Pending oxigdal-gpkg API stabilization.

- [ ] Raster calculator NumPy-expression evaluation (`oxigdal.calc("A * 2 + B", A=arr1, B=arr2)`)
  - **Goal:** Lazy evaluation of NumPy ufunc expressions on out-of-core rasters.
  - **Files:** `crates/oxigdal-python/src/expression.rs` (extend; already 44.8K).
...
- [ ] matplotlib/folium visualization helpers (`oxigdal.plot(ds)`, `oxigdal.show_on_folium(ds)`)
  - **Goal:** Pure-Python helpers that consume an `oxigdal.Dataset` for quick map preview.
  - **Files:** (new) `crates/oxigdal-python/python/oxigdal/_viz.py`.
  - **Why deferred:** No Rust dependency.

## Low Priority / Future (one-liners)
- [ ] Jupyter magic commands `%oxigdal_load`, `%oxigdal_plot` (delegate to oxigdal-jupyter).
- [ ] Dask array backend for chunked out-of-core raster ops.
- [ ] STAC client `oxigdal.stac.search(catalog, bbox=...)`.
- [ ] scikit-learn `oxigdal.ml.RasterTransformer` for tabular pipelines.
- [ ] Apache Arrow zero-copy exchange (`oxigdal.to_arrow()` / `from_arrow()`).
- [ ] QGIS Processing provider plugin (`processing.run("oxigdal:hillshade", ...)`).
- [ ] conda-forge recipe (`feedstock` for `oxigdal-python`).

## Cross-crate dependencies
- **Blocks:** None directly (downstream is Python ecosystem).
- **Blocked by:** oxigdal-cloud (remote URL Dataset), oxigdal-proj (EPSG/OGC parser), oxigdal-copc (point cloud), oxigdal-gpkg (vector GPKG).
