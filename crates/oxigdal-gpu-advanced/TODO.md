...
- [ ] Real 2-D FFT in `fft.wgsl`
  ...
  - **Files:** `crates/oxigdal-gpu-advanced/src/kernels/advanced/fft.wgsl` (rewrite `fft_2d_rows`, `fft_2d_cols`), `src/kernels/mod.rs::FftKernel` host-side dispatch.
...
- [ ] Multi-GPU data partitioning with overlap regions for convolution
  ...
  - **Design:** Add `ConvolutionPartitioner { radius: u32 }` that emits `Vec<Partition { device_id, rows: Range<usize>, halo_top: u32, halo_bottom: u32 }>`. Boundaries gathered post-execution via the cross-GPU gather path (see oxigdal-gpu Item 2). Edge devices have one-sided halo only.
  - **Files:** `crates/oxigdal-gpu-advanced/src/multi_gpu/load_balancer.rs` (extend), new `src/multi_gpu/partitioner.rs`.
  ...
  - **Prerequisites:** Cross-GPU gather (oxigdal-gpu).
...
- [ ] Peer-to-peer GPU memory copy with PCIe/NVLink detection
  ...
  - **Files:** `crates/oxigdal-gpu-advanced/src/multi_gpu/device_manager.rs`, new `src/multi_gpu/p2p.rs`.
...
- [ ] Real GPU timestamp queries in `GpuProfiler`
  ...
  - **Files:** `crates/oxigdal-gpu-advanced/src/profiling.rs` (extend `GpuProfiler::profile_pass`).
...
- [ ] Memory defragmentation with actual buffer migration in `MemoryCompactor`
  ...
  - **Files:** `crates/oxigdal-gpu-advanced/src/memory_compaction.rs` (replace any TODO/stub paths), `src/memory_pool.rs` (expose live-allocation iterator).
...
- [ ] Wire work-stealing queue to real wgpu command submission
  ...
  - **Files:** `crates/oxigdal-gpu-advanced/src/multi_gpu/work_queue.rs`.
...
- [ ] Shader optimizer passes beyond dead-code elimination
  ...
  - **Files:** `crates/oxigdal-gpu-advanced/src/shader_compiler/optimizer.rs`.
...
- [ ] Terrain WGSL kernels (slope, aspect, curvature) — actual shader code, not placeholders.
  ...
  - **Why deferred:** CPU reference exists in `oxigdal-terrain`; port after FFT lands.
- [ ] ML-inference WGSL kernels (matmul + activations).
  - **Files:** `src/gpu_ml/compute.rs`, `src/gpu_ml/neural.rs`.
  - **Why deferred:** Coordinate with oxigdal-ml CUDA/Metal EP roadmap.
...
- [ ] Network-distributed GPU cluster (RDMA/InfiniBand) — coordinate with oxigdal-cluster.
...
- **Blocks:** oxigdal-services (GPU compositing), oxigdal-ml (Vulkan/Metal EP wired to multi-GPU).
- **Blocked by:** oxigdal-gpu (this crate extends it), wgpu 29 timestamp/feature surface, OxiFFT (reference path).
