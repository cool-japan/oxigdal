//! Benchmarks for oxigeo-kafka

#![allow(missing_docs)]

use criterion::{Criterion, criterion_group, criterion_main};
use oxigeo_kafka::producer::{PartitionerType, create_partitioner};
...
fn bench_message_batching(c: &mut Criterion) {
    use bytes::Bytes;
    use oxigeo_kafka::producer::{BatchManager, MessageBatch};
...
fn bench_offset_management(c: &mut Criterion) {
    use oxigeo_kafka::consumer::{CommitStrategy, InMemoryOffsetStore, OffsetManager, OffsetStore};
...
fn bench_metrics(c: &mut Criterion) {
    use oxigeo_kafka::producer::{AtomicProducerMetrics, ProducerMetrics};
...
fn bench_schema_compatibility(_c: &mut Criterion) {
    ...
    {
        use oxigeo_kafka::schema_registry::CompatibilityChecker;
