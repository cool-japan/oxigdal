...
Apache Kafka integration for OxiGeo - High-performance async producer/consumer with schema registry and transactional support.
...
use oxigeo_kafka::config::{Acks, CompressionType};
...
```rust
use oxigeo_kafka::consumer::{ConsumerConfig, KafkaConsumer};
use oxigeo_kafka::config::{OffsetReset, IsolationLevel};
...
```rust
use oxigeo_kafka::transactions::{TransactionConfig, TransactionalProducer};
use std::time::Duration;
...
```rust
use oxigeo_kafka::schema_registry::{SchemaRegistry, SchemaRegistryConfig};
