...
- [ ] Replace `MockSyncProtocol` with a real HTTP/2 transport
  ...
  - **Files:** (new) `crates/oxigdal-edge/src/sync/http_protocol.rs`; `crates/oxigdal-edge/src/sync/manager.rs` (constructor dispatch); `crates/oxigdal-edge/src/sync/mod.rs` (re-export); `crates/oxigdal-edge/Cargo.toml` (add reqwest, uuid).
...
- [ ] Persistent SQLite (or sled) cache storage backend
  ...
  - **Files:** `crates/oxigdal-edge/src/cache.rs` (sled backend); (new) `crates/oxigdal-edge/src/cache/persistent.rs`; `crates/oxigdal-edge/Cargo.toml` (`oxicode` for serialization — replacing any bincode usage if found).
...
- [ ] Real CRDT merge for concurrent edit conflict resolution
  ...
  - **Files:** `crates/oxigdal-edge/src/conflict.rs` (validate / extend); (new) `crates/oxigdal-edge/src/conflict/or_set.rs`, `crates/oxigdal-edge/src/conflict/lww_map.rs`, `crates/oxigdal-edge/src/conflict/rga.rs`.
...
- [ ] Edge node discovery + mesh networking (mDNS + UDP gossip)
  ...
  - **Goal:** Edge nodes on the same LAN auto-discover each other via mDNS (`_oxigdal-edge._tcp.local`), exchange capability vectors, and form a gossip mesh for peer-to-peer cache sync that bypasses cloud round-trips.
  ...
  - **Files:** (new) `crates/oxigdal-edge/src/mesh/discovery.rs`, `crates/oxigdal-edge/src/mesh/gossip.rs`, `crates/oxigdal-edge/src/mesh/membership.rs`; modify `crates/oxigdal-edge/Cargo.toml` to add `mdns-sd`.
...
- [ ] Delta compression for sync payloads (rsync-style rolling hash)
  ...
  - **Files:** `crates/oxigdal-edge/src/compression.rs` (extend); (new) `crates/oxigdal-edge/src/compression/rdiff.rs`.
...
- [ ] Priority-based sync queue (critical items first, deferred uploads last)
  ...
  - **Files:** `crates/oxigdal-edge/src/sync/queue.rs` (new).
...
- [ ] Edge-side ML inference scheduling with model versioning
  ...
  - **Files:** (new) `crates/oxigdal-edge/src/ml/inference.rs`.
  - **Why deferred:** Coordinated with oxigdal-ml.

- [ ] Data retention policy with configurable TTL
  - **Goal:** Items older than `ttl_secs` get evicted regardless of LRU.
  - **Files:** `crates/oxigdal-edge/src/cache.rs` (extend with TTL sweep).
...
- [ ] Bandwidth-aware sync (defer big uploads to WiFi)
  ...
  - **Files:** `crates/oxigdal-edge/src/sync/manager.rs` (network check).
  - **Why deferred:** Requires network-type detection from oxigdal-mobile-enhanced.
...
- [ ] Write-ahead log for crash-safe local operations
  ...
  - **Files:** (new) `crates/oxigdal-edge/src/wal.rs`.
...
- [ ] Edge cluster coordination with Raft leader election
  ...
  - **Files:** (new) `crates/oxigdal-edge/src/cluster/raft.rs`.
...
- [ ] Adaptive compression algorithm selection by data type
  ...
  - **Files:** `crates/oxigdal-edge/src/compression.rs` (extend `AdaptiveCompressor`).
...
- **Blocks:** oxigdal-mobile-enhanced (sync infrastructure).
- **Blocked by:** oxigdal-ml (edge inference), oxigdal-mobile-enhanced (network-type detection for bandwidth-aware sync).

## Recently completed (verbatim)
- (no `[x]` entries in prior TODO.md — see README.md for the offline-first architecture; `.edge_minimal/` and `.oxigdal_cache/` directories indicate prior test runs)
