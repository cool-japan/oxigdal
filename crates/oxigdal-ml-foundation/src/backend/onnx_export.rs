...
impl OnnxGraph {
    ...
    pub fn to_model_bytes(&self) -> Vec<u8> {
        ...
        write_string_field(&mut buf, 2, "oxigdal-ml-foundation");
...
/// Magic prefix identifying an OxiGDAL weight checkpoint (`OXW1`).
...
pub fn deserialize_named_weights(bytes: &[u8]) -> Result<WeightMap> {
    ...
    if magic != WEIGHT_MAGIC {
        return Err(Error::Serialization(
            "weight checkpoint: bad magic (not an OxiGDAL weight file)".to_string(),
        ));
    }
...
mod tests {
    ...
    fn test_weight_checkpoint_file_roundtrip() {
        ...
        let path = std::env::temp_dir().join("oxigdal_ml_weights_test.oxw");
