...
- [ ] Unified connection pooling across backends (currently each backend's own pool API).
  ...
  - **Design:** Each backend wraps its native pool with `oxigdal-db-connectors::connection::PoolAdapter` implementing `async fn acquire(&self) -> Result<Conn>` and `fn stats(&self) -> PoolStats`. ClickHouse has no native pool — use `bb8` (already in deps).
...
- [ ] WKB/WKT geometry codec shared across non-PostGIS backends.
  ...
  - **Design:** Re-export `oxigdal-postgis::wkb` (already in deps) plus add `src/wkb_common.rs` for backend-specific framing (MySQL prefix: `[4-byte SRID LE][1-byte byte-order][4-byte type][...]` per MySQL 8.0 ref §11.4.3; SpatiaLite blob: magic `0x00` + `[1-byte endian][4-byte SRID][8-byte mbr_x_min]...[envelope]...[wkb]...[0xFE]`).
...
- [ ] Pool metrics export to `oxigdal-observability` (Prometheus gauges per backend).
- [ ] `mongodb_async_std` alternative runtime.

## Cross-crate dependencies
- **Blocks:** `oxigdal-services` (WFS Transaction backends), `oxigdal-query` (alternative `DataSource` impls).
- **Blocked by:** `oxigdal-postgis` (shared WKB encoder).
