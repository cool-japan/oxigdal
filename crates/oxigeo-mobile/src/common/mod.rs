...
mod tests {
    ...
    fn test_prefetch_tiles_invalid_bbox() {
        // null bbox
        let result =
            unsafe { oxigeo_mobile_prefetch_tiles(std::ptr::null(), std::ptr::null(), 0, 5) };
        assert_eq!(result, -1);
    }
    ...
    fn test_prefetch_tiles_invalid_zoom() {
        ...
        let result = unsafe { oxigeo_mobile_prefetch_tiles(std::ptr::null(), &bbox, -1, 5) };
        assert_eq!(result, -1);

        // Invalid zoom (max < min)
        let result = unsafe { oxigeo_mobile_prefetch_tiles(std::ptr::null(), &bbox, 5, 2) };
        assert_eq!(result, -1);
    }
    ...
    fn test_prefetch_tiles_small_bbox() {
        ...
        let result = unsafe { oxigeo_mobile_prefetch_tiles(std::ptr::null(), &bbox, 0, 2) };
        assert!(result >= 0);
    }
    ...
    fn test_prefetch_tiles_offline_mode() {
        ...
        let result = unsafe { oxigeo_mobile_prefetch_tiles(std::ptr::null(), &bbox, 0, 1) };
