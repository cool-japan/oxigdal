...
- [ ] Push-subscription endpoint registration + webhook verification (`X-Goog-Channel-Id`, JWT verification)
  - **Goal:** Allow OxiGDAL services to receive push deliveries instead of pull-only.
  - **Files:** new `src/subscriber/push.rs` (currently `subscriber.rs` is monolithic 28 KB pull-only).
  - **Why deferred:** Requires HTTP server integration; pair with oxigdal-services webhook path.
...
- [ ] Bridge into oxigdal-streaming (`PubSubSource` with watermark)

## Cross-crate dependencies
- **Blocks:** oxigdal-streaming (durable cloud source/sink)
