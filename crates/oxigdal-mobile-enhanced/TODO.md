...
- [ ] Real battery state reading via platform APIs (replace mock `75.0%`)
  ...
  - **Design:** Split per-OS:
    ...
    - **Android** (`cfg(target_os = "android")`): use JNI to query `BatteryManager` (level, scale, status, temperature) via existing `oxigdal-mobile` JNI bridge.
  ...
  - **Files:** `crates/oxigdal-mobile-enhanced/src/battery.rs` (replace mock block); (new) `crates/oxigdal-mobile-enhanced/src/ios/battery.rs`; (new) `crates/oxigdal-mobile-enhanced/src/android/battery.rs`; (new) `crates/oxigdal-mobile-enhanced/src/linux/battery.rs`.
...
- [ ] Real network type detection (replace `Ok(NetworkType::WiFi)` mock)
  ...
  - **Files:** `crates/oxigdal-mobile-enhanced/src/network.rs` (replace mock); (new) `crates/oxigdal-mobile-enhanced/src/ios/network.rs`; (new) `crates/oxigdal-mobile-enhanced/src/android/network.rs`.
...
- [ ] Real bandwidth measurement to replace `Mock quality metrics` block
  ...
  - **Files:** `crates/oxigdal-mobile-enhanced/src/network.rs` (replace mock); add `reqwest = { workspace = true, default-features = false, features = ["rustls-tls"], optional = true }` gated by feature `bandwidth-probe`.
...
- [ ] LZ4 compression real implementation (currently falls back to deflate)
  ...
  - **Design:** Use `oxiarc-lz4` (already in workspace per `oxigdal-edge/Cargo.toml`). Add `oxiarc-lz4 = { workspace = true }` to `oxigdal-mobile-enhanced/Cargo.toml`. Call `oxiarc_lz4::compress(data, level)` and `oxiarc_lz4::decompress(data)`. Compression level 1-12 (LZ4 frame format default 1; HC mode 4+).
  - **Files:** `crates/oxigdal-mobile-enhanced/src/network.rs` (real call); `crates/oxigdal-mobile-enhanced/Cargo.toml` (add dep).
  ...
  - **Risk:** None significant — oxiarc-lz4 is already a stable workspace dep used by oxigdal-edge.
  - **Prerequisites:** None.
...
- [ ] Real Android RAM availability replacing mocked `total_ram / 3`
  ...
  - **Files:** `crates/oxigdal-mobile-enhanced/src/android/memory.rs`, `crates/oxigdal-mobile-enhanced/src/ios/memory.rs`, `crates/oxigdal-mobile-enhanced/src/storage/mod.rs`, `crates/oxigdal-mobile-enhanced/src/android/mod.rs`, `crates/oxigdal-mobile-enhanced/src/ios/mod.rs`.
...
- [ ] Background task `enqueue` placeholder needs WorkManager/BGTaskScheduler integration
  ...
  - **Files:** `crates/oxigdal-mobile-enhanced/src/background.rs` (extract trait); (new) `crates/oxigdal-mobile-enhanced/src/android/background.rs`; (new) `crates/oxigdal-mobile-enhanced/src/ios/background.rs`.
...
- [ ] Storage compression entropy estimator: replace "simplified" with Shannon entropy
  ...
  - **Files:** `crates/oxigdal-mobile-enhanced/src/storage/compression.rs`.
...
- [ ] iOS Metal GPU acceleration hints for raster processing
  ...
  - **Files:** (new) `crates/oxigdal-mobile-enhanced/src/ios/metal.rs`.
  - **Why deferred:** Cross-cuts with oxigdal-gpu.
...
- [ ] Android RenderScript fallback for image processing on legacy devices
  ...
  - **Files:** (new) `crates/oxigdal-mobile-enhanced/src/android/renderscript.rs`.
...
- [ ] Memory pressure handler for iOS `didReceiveMemoryWarning` and Android `onTrimMemory`
  ...
  - **Files:** `crates/oxigdal-mobile-enhanced/src/android/memory.rs`, `crates/oxigdal-mobile-enhanced/src/ios/memory.rs`.
...
- [ ] Geofencing-aware prefetch (download tiles for known routes)
  ...
  - **Files:** (new) `crates/oxigdal-mobile-enhanced/src/geofencing.rs`.
...
- [ ] Thermal state monitoring (throttle on `processInfo.thermalState` / `ThermalService`)
  ...
  - **Files:** (new) `crates/oxigdal-mobile-enhanced/src/thermal.rs`.
...
- [ ] Adaptive sync protocol for partial dataset updates
  ...
  - **Files:** (new) `crates/oxigdal-mobile-enhanced/src/sync.rs`.
  - **Why deferred:** Coordinated with oxigdal-edge sync work.

- [ ] Offline-first vector tile rendering with local cache
  - **Goal:** MVT rendering using cached tiles only; no network fall-through.
  - **Files:** (new) `crates/oxigdal-mobile-enhanced/src/offline_mvt.rs`.
  - **Why deferred:** Pending oxigdal-mvt extraction.
...
- **Blocks:** Mobile apps using both oxigdal-mobile and oxigdal-mobile-enhanced.
- **Blocked by:** oxigdal-mobile (FFI base layer), oxigdal-edge (sync infra), oxigdal-mvt (planned extraction), oxigdal-gpu (Metal interop).
