...
async fn test_full_backup() {
    let backup_dir = std::env::temp_dir().join("oxigeo-ha-test-full-backup");
...
async fn test_incremental_backup() {
    let backup_dir = std::env::temp_dir().join("oxigeo-ha-test-incr-backup");
    tokio::fs::create_dir_all(&backup_dir).await.ok();
...
async fn test_differential_backup() {
    let backup_dir = std::env::temp_dir().join("oxigeo-ha-test-diff-backup");
    tokio::fs::create_dir_all(&backup_dir).await.ok();
