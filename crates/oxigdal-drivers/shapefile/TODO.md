# TODO: oxigdal-drivers/shapefile

> **Purpose:** Shapefile (ESRI) driver for OxiGDAL - Pure Rust GDAL reimplementation
...
- [ ] Implement CRS reprojection during read / write
  ...
  - **Design:** Wire `oxigdal-proj::Transformer` (workspace dep) behind a `reproject` feature flag. Build pipeline once from `.prj` WKT + target EPSG; apply per-vertex during read iteration; rewrite `.prj` on write. Spec: ISO 19162 WKT2 + EPSG codes.
  - **Files:** (new) `src/reproject.rs`, `Cargo.toml` (add `oxigdal-proj.workspace = true` under feature gate)
