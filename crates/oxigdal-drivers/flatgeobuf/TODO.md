# TODO: oxigdal-drivers/flatgeobuf
...
- [ ] Add CRS reprojection during read/write via `oxigdal-proj`
  ...
  - **Design:** Use `oxigdal-proj::Transformer` (workspace dep). Per-feature coordinate iterator -> transform -> rebuild geometry. Spec: WKT2 (ISO 19162) CRS strings; `oxigdal-proj` already handles EPSG codes.
  - **Files:** (new) `src/reproject.rs`, `Cargo.toml` (add `oxigdal-proj.workspace = true` under a `reproject` feature gate to keep base crate light)
