...
- [ ] Add GeoJP2 metadata box (UUID `B14BF8BD-083D-4B43-A5AE-8CD7D5A6CE03`) reading and writing for georeferenced JP2
  ...
  - **Design:** GeoJP2 stores a degenerate GeoTIFF (no image data) in a UUID box. Detect UUID = `B14BF8BD083D4B43A5AE8CD7D5A6CE03`; parse the embedded TIFF tags using existing oxigdal-geotiff tag parser; expose as `Jp2Metadata::geo_transform`, `geo_crs`, `geo_tiepoints`. Write mirror: serialize tags to TIFF, wrap in UUID box, emit before jp2c. Spec: OGC 05-047r3 ("GMLJP2 v1.0" - GeoJP2 referenced therein).
  ...
  - **Prerequisites:** Depends on `oxigdal-geotiff` exposing its tag parser as a library function (likely already does).
...
- **Blocks:** `oxigdal-drivers/grib` (GRIB2 Template 5.40 needs a JPEG2000 decoder), `oxigdal-cog` (COG can use JPEG2000 compression per GeoTIFF tag 34712).
- **Blocked by:** None.
