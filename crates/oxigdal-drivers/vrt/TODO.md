# TODO: oxigdal-drivers/vrt

> **Purpose:** VRT (Virtual Raster) driver for OxiGDAL - Pure Rust GDAL reimplementation
...
- [ ] Implement warped VRT for on-the-fly reprojection
  ...
  - **Design:** Parse `<GDALWarpOptions>` from XML: `<Transformer>` (typically `<GenImgProjTransformer>` with `SrcGeoTransform`, `DstGeoTransform`, `SrcSRS`, `DstSRS`), `<ResampleAlg>` (`NearestNeighbour` / `Bilinear` / `Cubic` / `Lanczos`). For each destination pixel, inverse-transform to source pixel via `oxigdal-proj`, read source value with chosen resampler. Spec: GDAL VRT Warped subclass documentation.
  - **Files:** (new) `src/warp.rs`, `src/reader.rs` (dispatch on `VrtSubclass`), `src/xml.rs` (parse `GDALWarpOptions`), `Cargo.toml` (add `oxigdal-proj.workspace = true` under a `warp` feature)
  ...
  - **Risk:** Resampler accuracy at edges; reuse `oxigdal-warp` crate if it exists, otherwise inline.
  - **Prerequisites:** `oxigdal-proj` and possibly `oxigdal-warp` if separate.
...
- [ ] Implement multi-source compositing priority / overlap resolution
  ...
  - **Risk:** Data-type-specific reducer (integer max differs from float max with NaN); use `oxigdal-core::RasterDataType` dispatch.
  - **Prerequisites:** None.
...
- [ ] VRT validation (source-file existence check, dimension consistency check)
  ...
  - **Design:** Walk `VrtDataset.bands`; for each `VrtSource`: check `source_filename.exists()` (absolute or relative path); open the source via `oxigdal-geotiff` lazily; compare source `width`/`height` to declared `<SrcRect>`; tabulate issues in `Vec<ValidationIssue>`.
...
- **Blocked by:** `oxigdal-proj` (for warp item only), `oxigdal-geotiff` (for source reading).
...
- [x] Lazy tile reading from source rasters — `VrtReader::read_window` at `src/reader.rs:124` reads only the requested window from each contributing source via `oxigdal_geotiff::read_band`; verified 2026-05-16.
