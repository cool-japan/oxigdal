...
- [ ] Implement NetCDF-4 reader metadata extraction (currently returns `NetCdf4NotAvailable`)
  - **Verified gap:** `src/reader.rs:221` - `// NetCDF-4 support is placeholder for now` followed by `Err(NetCdfError::NetCdf4NotAvailable)`. The function `read_metadata_nc4` is the only entry point for NC4 reads and always fails. Crate has its own Pure-Rust HDF5 parser in `src/netcdf4/` (look at module exports in `src/lib.rs:248-251`: `Hdf5Superblock`, `Hdf5SuperblockVersion`, `Nc4Reader`, etc.) so the building blocks exist; they just are not glued into the high-level `NetCdfReader` path.
...
- [ ] Implement NetCDF-4 writer (currently returns `NetCdf4NotAvailable`)
  - **Verified gap:** `src/writer.rs:116` - `// NetCDF-4 support is placeholder for now`. Symmetric to the reader gap.
...
- [ ] Implement `_FillValue` / `missing_value` attribute handling
  - **Verified gap:** `src/cf_conventions/` and `src/reader.rs` have no fill-value substitution path. `rg -n "fill_value|_FillValue|missing_value" -g '*.rs' src/` shows mentions only in comments and tests.
...
- [ ] Variable packing/unpacking (scale_factor, add_offset)
...
- [ ] Coordinate bounds variable support (cell boundaries)
...
- [ ] Pure Rust NetCDF-4 reader (subset of HDF5 for NC4 files - much of this already exists in `src/netcdf4/`)
...
- **Blocks:** `oxigdal-cli` (NC subcommand richness), `oxigdal-drivers/grib` (GRIB-to-NC conversion).
