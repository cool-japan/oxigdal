...
pub fn validate_cf_version(conventions: &str) -> Option<CfVersion> {
    ...
    for part in conventions.split(',') {
        ...
        if let Some(stripped) = part.strip_prefix("CF-") {
            if let Some(v) = CfVersion::from_str(stripped) {
...
pub fn validate_units(units: &str) -> bool {
    ...
    // Accept if it contains at least one known SI base unit symbol
    const KNOWN: &[&str] = &["K", "Pa", "m", "s", "kg", "mol", "W", "J", "N", "Hz", "A"];
    KNOWN.iter().any(|b| units.contains(b))
}
...
mod tests {
    ...
    fn test_cf_version_from_str_valid() {
        let v = CfVersion::from_str("1.11").unwrap();
        assert_eq!(v, CfVersion::v1_11());
    }

    #[test]
    fn test_cf_version_from_str_invalid() {
        assert!(CfVersion::from_str("abc").is_none());
        assert!(CfVersion::from_str("1").is_none());
        assert!(CfVersion::from_str("").is_none());
    }
    ...
    fn test_cf_version_ordering() {
        ...
        assert_eq!(CfVersion::v1_8(), CfVersion::from_str("1.8").unwrap());
    }
    ...
    fn test_lookup_standard_name_found() {
        let entry = lookup_standard_name("air_temperature").unwrap();
    ...
    fn test_geometry_type_cf_names_roundtrip() {
        ...
        for t in &types {
            let name = t.cf_name();
            let back = CfGeometryType::from_cf_name(name).unwrap();
            assert_eq!(&back, t);
        }
    }
    ...
    fn test_cell_method_from_str_known() {
        assert_eq!(CellMethodName::from_str("mean"), CellMethodName::Mean);
        assert_eq!(CellMethodName::from_str("maximum"), CellMethodName::Maximum);
        assert_eq!(
            CellMethodName::from_str("standard_deviation"),
    ...
    fn test_cell_method_from_str_unknown() {
        let u = CellMethodName::from_str("trimmed_mean");
        assert!(!u.is_known());
    ...
    fn test_cell_method_roundtrip() {
        ...
        for &m in &methods {
            let parsed = CellMethodName::from_str(m);
    ...
    fn test_validate_cf_version_simple() {
        let v = validate_cf_version("CF-1.11").unwrap();
        assert_eq!(v, CfVersion::v1_11());
    }

    #[test]
    fn test_validate_cf_version_with_acdd() {
        let v = validate_cf_version("CF-1.8, ACDD-1.3").unwrap();
    ...
    fn test_validate_cf_version_multiple_cf() {
        ...
        let v = validate_cf_version("CF-1.6, CF-1.11").unwrap();
        assert_eq!(v, CfVersion::v1_11());
    }
