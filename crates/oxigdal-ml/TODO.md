...
- [ ] Re-enable `coreml` feature against `objc2` 0.6
  ...
  - **Files:** `crates/oxigdal-ml/src/gpu.rs` (CoreML adapter section), `Cargo.toml` (uncomment `coreml` feature + deps).
...
- [ ] Async WebGPU device enumeration
  ...
  - **Files:** `crates/oxigdal-ml/src/gpu.rs` (extend `enumerate_webgpu_devices` block).
...
- [ ] Real TFLite quantization scale/zero-point extraction
  ...
  - **Files:** `crates/oxigdal-ml/src/models/tflite.rs` (lines 420-436 plus `TensorInfo` struct), upstream blocker noted in `Cargo.toml:32-35`.
...
- [ ] Re-enable `temporal` feature behind clean workspace deps
  - **Verified gap:** `Cargo.toml:44-45` — `# TEMPORARILY DISABLED: Optional dependency resolution issues with workspace features / # temporal = ["dep:scirs2-series", "dep:oxigdal-ml-foundation", ...]`
  - **Goal:** Time-series ML for change detection / forecasting (`TemporalForecaster`, `ConvLSTM`) without breaking workspace resolution.
  - **Design:** Move `oxigdal-ml-foundation` and `oxigdal-temporal` optional-deps into `[target.'cfg(any())'.dependencies]`-style virtual gate or split a `oxigdal-ml-temporal` adapter crate that depends on both unconditionally; expose only the re-exported `TemporalForecaster` via `#[cfg(feature = "temporal")] pub mod temporal;` (already wired at `src/lib.rs:317-334`).
  - **Files:** `Cargo.toml`, `crates/oxigdal-ml/src/temporal/` (existing).
  - **Tests:** (proposed) `test_temporal_forecaster_compiles_under_feature`, `test_temporal_convlstm_one_step_ahead`.
  - **Risk:** `oxigdal-ml-foundation?/ml` cycle — needs upstream foundation crate's `ml` feature surface stabilized first.
  - **Prerequisites:** oxigdal-ml-foundation backend stabilization (sibling crate, see its TODO).
...
- [ ] Model quantization (INT8/FP16) with accuracy regression suite
  ...
  - **Files:** `crates/oxigdal-ml/src/optimization/quantization.rs` (new), wire into `OptimizationPipeline`.
...
- [ ] Connect remaining GPU backends to real runtime APIs
  ...
  - **Files:** `crates/oxigdal-ml/src/gpu.rs`.
...
- [ ] Streaming inference for continuous satellite-imagery feeds (S3/HTTP range-based tile fetch + circular tile buffer + back-pressure).
  - **Files:** `crates/oxigdal-ml/src/inference/streaming.rs` (new).
  - **Why deferred:** Needs cloud-native transport layer (cross-crate); coordinates with `oxigdal-streaming` v0.2.
- [ ] Model ensemble support (voting, stacking, blending).
  - **Files:** `crates/oxigdal-ml/src/inference/ensemble.rs` (new).
...
- [ ] Active-learning loop with uncertainty sampling from segmentation outputs.
  - **Files:** `crates/oxigdal-ml/src/inference/active_learning.rs` (new).
...
- [ ] Real model versioning with registry, semver, and rollback.
  - **Files:** `crates/oxigdal-ml/src/model_versioning.rs` (extend existing).
...
- [ ] Geospatial-aware augmentations (north-up rotation, scale-aware crop, spectral band dropout) wired to `scirs2-core::random`.
  - **Files:** `crates/oxigdal-ml/src/augmentation/` (extend).
...
- [ ] Change-detection pipeline (bi-temporal differencing + threshold optimization).
  - **Files:** `crates/oxigdal-ml/src/inference/change_detection.rs` (new).
  - **Why deferred:** Overlaps with `oxigdal-analytics::change`; coordinate with analytics crate.
- [ ] Model explainability (GradCAM, SHAP) for classification heads.
  - **Files:** `crates/oxigdal-ml/src/inference/explainability.rs` (new).
...
- [ ] Real model zoo with HTTP download + SHA-256 verification + local cache.
  - **Files:** `crates/oxigdal-ml/src/zoo.rs` (extend; deps already include `reqwest` + `sha2`).
...
- **Blocks:** oxigdal (umbrella), oxigdal-services (ML inference endpoints), oxigdal-jupyter (ML notebooks).
- **Blocked by:** oxigdal-ml-foundation (training backend), oxigdal-temporal (`temporal` feature), oxionnx (CoreML/Vulkan/OpenCL EP coverage).
