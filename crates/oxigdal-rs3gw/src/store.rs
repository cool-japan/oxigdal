...
impl Rs3gwStore {
    ...
    /// [`AsyncStore::set`](oxigdal_zarr::storage::AsyncStore::set) is encrypted
...
impl Rs3gwStore {
    ...
    #[cfg(feature = "encryption")]
    fn encode_value(&self, key: &str, value: &[u8]) -> oxigdal_zarr::error::Result<Vec<u8>> {
    ...
    fn encode_value(&self, _key: &str, value: &[u8]) -> oxigdal_zarr::error::Result<Vec<u8>> {
        Ok(value.to_vec())
    }
    ...
    /// returns the bytes unchanged.
    #[cfg(feature = "encryption")]
    fn decode_value(&self, key: &str, raw: Vec<u8>) -> oxigdal_zarr::error::Result<Vec<u8>> {
        match self.active_encryption(key) {
    ...
    /// Non-encryption build: payloads pass through untouched.
    #[cfg(not(feature = "encryption"))]
    fn decode_value(&self, _key: &str, raw: Vec<u8>) -> oxigdal_zarr::error::Result<Vec<u8>> {
        Ok(raw)
    }
}
...
impl Store for Rs3gwStore {
    ...
    fn get(&self, key: &StoreKey) -> oxigdal_zarr::error::Result<Vec<u8>> {
        ...
        // Try to get current runtime handle
        let fetched: oxigdal_zarr::error::Result<Vec<u8>> =
            match tokio::runtime::Handle::try_current() {
                Ok(_handle) => {
                    // We're already in a tokio runtime, use block_in_place
                    tokio::task::block_in_place(|| {
