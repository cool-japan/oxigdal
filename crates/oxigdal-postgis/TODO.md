...
- [x] Replace the "COPY → individual INSERTs" fallback in `PostGisWriter::flush` with a real PostgreSQL binary COPY stream.
  ...
  - **Tests:** 10 in `crates/oxigdal-postgis/tests/copy_binary_test.rs` — 9 active byte-layout encoder tests (signature, header flags, field count, length prefix BE, null, trailer, multi-row, EWKB SRID flag, EWKB Point round trip) + 1 `#[ignore]` live-DB round-trip needing PostgreSQL+PostGIS. Build + clippy clean; 9 pass, 1 skipped.
...
- [ ] Pool metrics export (active/idle/waiting) for `oxigdal-observability`.
...
- **Blocks:** `oxigdal-services` (WFS Transaction execution), `oxigdal-query` (PostGIS `DataSource`), `oxigdal-db-connectors` (TimescaleDB connector inherits from this crate).
- **Blocked by:** None.
