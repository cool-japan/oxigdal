...
async fn database_client(db: &DatabaseSource) -> ServiceResult<deadpool_postgres::Object> {
    let pool = oxigdal_postgis::ConnectionPool::from_connection_string(&db.connection_string)
...
fn postgis_unavailable(operation: &str) -> ServiceError {
    ServiceError::Transaction(format!(
        "Database transaction '{operation}' is unavailable: oxigdal-services was built \
...
mod tests {
    ...
    fn unique_geojson_path() -> PathBuf {
        std::env::temp_dir().join(format!("oxigdal_wfs_tx_{}.geojson", uuid::Uuid::new_v4()))
    }
